...
engine = create_engine(os.environ.get('DATABASE_URL', 'sqlite:///test.db'),echo=False)#,connect_args={'check_same_thread':False})
...
class Read(db.Model):
    __tablename__ = 'readtable'

    id = db.Column(db.Integer, primary_key=True)
    stamp = db.Column(db.String(26))
#    devId = db.Column(db.String(15))
    getdata = db.Column(db.String(20))
#    SPA = db.Column(db.String(10))
#    TA = db.Column(db.String(10))

    def __repr__(self):
        return '<User %r %r>' % (self.stamp, self.getdata)
db.create_all()
...
#        admin = Read(stamp=str(datetime.now()+timedelta(minutes=330)),getdata=str(message))
#        db.session.add(admin)
#        db.session.commit()
client = mqtt.Client()
print("client=",client)
...
#        global message
        ...
        time.sleep(2)
        message=engine.execute('select * from readtable', [1]).first()
        message=Read.query.filter_by().first()
        return "published {} READ:{} for getting reading = {}".format(val1,val2,message)
@app.callback(
